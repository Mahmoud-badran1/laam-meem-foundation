...
export function PromoMarquee() {
  return (
    <div className="overflow-hidden bg-[#d71920] text-white" role="region" aria-label="Winter Sale promotion">
      <Link
        to="/contact"
        className="group block border-b border-white/15 py-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-white"
      >
            ...
            <div key={track} className="flex shrink-0 items-center" aria-hidden={track === 1 ? true : undefined}>
              {promoItems.map((item) => (
                <div key={`${track}-${item}`} className="flex items-center gap-5 px-5 sm:gap-7 sm:px-7">
                  ...
                  <span className="text-[10px] font-medium uppercase tracking-[0.16em] opacity-90 sm:text-[11px]">
                    30% Off
                  </span>
